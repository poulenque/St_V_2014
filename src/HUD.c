...
void weapon_HUD(Game* game){
	...
	glRotated(60, 0, 1, 0);
	glRotated(60, 1, 0, 0);
	glTranslated(0,0,-5+1*oscill_force);
	// glTranslated(-.3+.1*sin(time_*.034),1+1*sin(cos(time_*.021)),0);
	glTranslated(-.3+.1*sin(SDL_GetTicks()*.0017),1,0);
...
void weapon_HUD_ARM(Game* game){
	...
	viseur(game);
	double time_pos=game->trigger_value/game->trigger_value_MAX;
	...
	BOW_GL_MATRIX();
			...
			TEMP_MACRO();
			draw_hand(.5*time_pos,2*time_pos,TRUE);
			draw_bow(.2,.3+1*time_pos);
...
void weapon_HUD_FIRE(Game* game){
	...
	//force entre 0 et 1,
	//correspond a avancement

	// double force=(time_-fire_anim_begin)/fire_anim_duration;
	double time_pos=game->trigger_value/game->trigger_value_MAX;
